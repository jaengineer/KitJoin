...
public class ActionBar extends FrameLayout {
    ...
    private void createBackButtonImage() {
        ...
        addView(backButtonImageView, layoutUtils.createFrame(54, 54, Gravity.LEFT | Gravity.TOP));
    ...
    private void createSubtitleTextView() {
        ...
        addView(subTitleTextView, layoutUtils.createFrame(layoutUtils.WRAP_CONTENT, layoutUtils.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP));
    }
    ...
    private void createTitleTextView() {
        ...
        titleTextView.setTypeface(Utilities.getTypeface("fonts/rmedium.ttf"));
        addView(titleTextView, layoutUtils.createFrame(layoutUtils.WRAP_CONTENT, layoutUtils.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP));
    }
    ...
    public ActionBarMenu createMenu() {
        ...
        layoutParams.height = layoutUtils.MATCH_PARENT;
        layoutParams.width = layoutUtils.WRAP_CONTENT;
    ...
    public ActionBarMenu createActionMode() {
        ...
        addView(actionMode);
        actionMode.setPadding(0, occupyStatusBar ? Utilities.statusBarHeight : 0, 0, 0);
        ...
        if (occupyStatusBar && actionModeTop == null) {
            ...
            layoutParams.height = Utilities.statusBarHeight;
            layoutParams.width = layoutUtils.MATCH_PARENT;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(width, actionBarHeight + extraHeight + (occupyStatusBar ? Utilities.statusBarHeight : 0));

        int textLeft;
        if (backButtonImageView != null && backButtonImageView.getVisibility() != GONE) {
            backButtonImageView.measure(MeasureSpec.makeMeasureSpec(Utilities.dp(54), MeasureSpec.EXACTLY), actionBarHeightSpec);
            textLeft =Utilities.dp(Utilities.isTablet() ? 80 : 72);
        } else {
            textLeft = Utilities.dp(Utilities.isTablet() ? 26 : 18);
        }

        if (menu != null && menu.getVisibility() != GONE) {
            int menuWidth;
            if (isSearchFieldVisible) {
                menuWidth = MeasureSpec.makeMeasureSpec(width - Utilities.dp(Utilities.isTablet() ? 74 : 66), MeasureSpec.EXACTLY);
            } else {
        ...
        if (titleTextView != null && titleTextView.getVisibility() != GONE || subTitleTextView != null && subTitleTextView.getVisibility() != GONE) {
            int availableWidth = width - (menu != null ? menu.getMeasuredWidth() : 0) -Utilities.dp(16) - textLeft;

            if (titleTextView != null && titleTextView.getVisibility() != GONE) {
                titleTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, !Utilities.isTablet() && getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE ? 18 : 20);
            ...
            if (subTitleTextView != null && subTitleTextView.getVisibility() != GONE) {
                subTitleTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, !Utilities.isTablet() && getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE ? 14 : 16);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        int additionalTop = occupyStatusBar ? Utilities.statusBarHeight : 0;
        ...
        if (menu != null && menu.getVisibility() != GONE) {
            int menuLeft = isSearchFieldVisible ? Utilities.dp(Utilities.isTablet() ? 74 : 66) : (right - left) - menu.getMeasuredWidth();
            menu.layout(menuLeft, additionalTop, menuLeft + menu.getMeasuredWidth(), additionalTop + menu.getMeasuredHeight());
        }

        int offset = Utilities.dp(!Utilities.isTablet() && getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE ? 1 : 2);
        if (titleTextView != null && titleTextView.getVisibility() != GONE) {
            int textTop;
            if (subTitleTextView != null && subTitleTextView.getVisibility() != GONE) {
                textTop = (getCurrentActionBarHeight() / 2 - titleTextView.getMeasuredHeight()) / 2 + offset;
            } else {
                textTop = (getCurrentActionBarHeight() - titleTextView.getMeasuredHeight()) / 2 - Utilities.dp(1);
            }
    ...
    public static int getCurrentActionBarHeight() {
        if (Utilities.isTablet()) {
            return Utilities.dp(64);
        } else if (init.applicationContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            return Utilities.dp(48);
        } else {
            return Utilities.dp(56);
        }
    }
}